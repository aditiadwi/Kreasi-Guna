...
| **Testimonial Card Modern Header-Layout & Compact Sizing** | Layout kartu ulasan publik sebelumnya menaruh bintang rating di paling atas dan nama pengulas di paling bawah dengan ukuran kartu yang agak terlalu lebar. | Memperbarui `renderTestimonials()` di `app.js` dengan struktur header terpisah: **Nama Pengulas** di kiri dan **Bintang Rating (★)** di kanan pada baris atas, diikuti pesan ulasan di baris bawah. Menyesuaikan CSS grid `.testimonials-grid` dan `.testimonial-card` di `style.css` agar kartu lebih compact (`minmax(260px, 1fr)`), simetris, dan rapi di Home page. |
